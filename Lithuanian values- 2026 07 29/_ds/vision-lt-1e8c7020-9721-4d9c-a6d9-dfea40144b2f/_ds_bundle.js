/* @ds-bundle: {"format":4,"namespace":"VisionLtDesignSystem_1e8c70","components":[],"sourceHashes":{"assets/icons.js":"62733862e413","assets/image-slot.js":"9309434cb09c","consent.js":"492ad280d13b","publish/assets/icons.js":"62733862e413","publish/assets/image-slot.js":"9309434cb09c","publish/consent.js":"492ad280d13b","vedlys/doc-page.js":"7feb7a4009b0","vedlys/tweaks-panel.jsx":"6591467622ed"},"inlinedExternals":[],"unexposedExports":[]} */

(() => {

const __ds_ns = (window.VisionLtDesignSystem_1e8c70 = window.VisionLtDesignSystem_1e8c70 || {});
...
// assets/icons.js
try { (() => {
/* vision.lt icon set — lucide 0.460.0 subset, self-hosted */
(function () {
  var I = {
    ...
    "flashlight": '<path d="M18 6c0 2-2 2-2 4v10a2 2 0 0 1-2 2h-4a2 2 0 0 1-2-2V10c0-2-2-2-2-4V2h12z"></path><line x1="6" x2="18" y1="6" y2="6"></line><line x1="12" x2="12" y1="12" y2="12"></line>',
...
// consent.js
try { (() => {
/* vision.lt — cookie consent + tracker loaders (per-category, GDPR opt-in)
   Categories:
     analytics → Google Analytics 4 (G-5SG1YF6YPC) + Microsoft Clarity (xp34gsdhos)
     marketing → Facebook Pixel (DISABLED until FB_PIXEL_ID is set below)
   TO LAUNCH THE FB PIXEL: set FB_PIXEL_ID to your pixel id. Nothing else.
   The banner then re-prompts existing visitors (their analytics choice is kept;
   marketing is undecided for them until they answer again).
   Choices persist in localStorage ("vz_consent_v2" as JSON per category).
   API on every page: vzConsent.accept() / .reject() / .set(cat, 'granted'|'denied') / .status() */
(function () {
  var GA_ID = 'G-5SG1YF6YPC';
  var CLARITY_ID = 'xp34gsdhos'; // Microsoft Clarity — loads with analytics consent
  var FB_PIXEL_ID = ''; // e.g. '1234567890' — empty string keeps the marketing category off

  var CATS = ['analytics'];
  if (FB_PIXEL_ID) CATS.push('marketing');

  /* Per-page config via <script src="consent.js" data-lang="lt" data-privacy-href="../privatumas.html#slapukai">.
     Defaults keep existing EN pages unchanged. */
  var SCRIPT_DS = document.currentScript && document.currentScript.dataset || {};
  var LANG = SCRIPT_DS.lang === 'lt' ? 'lt' : 'en';
  var PRIVACY_HREF = SCRIPT_DS.privacyHref || 'privacy.html#cookies';
  var KEY = 'vz_consent_v2';
  var LEGACY_KEY = 'vz_consent';
  function readState() {
    var s = null;
    try {
    ...
    if (!s) {
      s = {};
      try {
        // migrate the old single-value consent (was analytics-only)
        var old = localStorage.getItem(LEGACY_KEY);
        if (old === 'granted' || old === 'denied') s.analytics = old;
      } catch (e) {}
    }
    return s;
  }
  ...
  function clearCookies(cat) {
    ...
    for (var i = 0; i < parts.length; i++) {
      var n = parts[i].split('=')[0].trim();
      var isGA = n === '_ga' || n === '_gid' || n.indexOf('_ga_') === 0;
      var isClarity = n === '_clck' || n === '_clsk' || n === 'CLID' || n === 'MUID';
      var isFB = n === '_fbp' || n === '_fbc';
      if (cat === 'analytics' && (isGA || isClarity) || cat === 'marketing' && isFB || !cat && (isGA || isClarity || isFB)) expire(n);
    }
  }
  function applyState() {
    var s = readState();
    CATS.forEach(function (c) {
      if (s[c] === 'granted') LOADERS[c]();
    });
  }
  ...
  function injectStyles() {
    if (document.getElementById('cookie-note-css')) return;
    var css = ['#cookie-note{position:fixed;left:clamp(1rem,3vw,2rem);bottom:clamp(1rem,3vw,2rem);z-index:200;max-width:min(370px,calc(100vw - 2rem));', 'background:#16130F;color:#D9D2C7;border:1px solid rgba(244,240,233,0.16);border-radius:18px;', 'padding:1.15rem 1.25rem;box-shadow:0 24px 60px -24px rgba(0,0,0,0.55);', 'font-size:0.92rem;line-height:1.5;opacity:0;transform:translateY(14px);', 'transition:opacity .4s ease,transform .4s cubic-bezier(0.22,1,0.36,1);}', '#cookie-note.show{opacity:1;transform:none;}', '#cookie-note p{margin:0 0 0.9rem;}', '#cookie-note a{color:#F4F0E9;}', '#cookie-note .cn-actions{display:flex;align-items:center;gap:0.6rem;}', '#cookie-note .cn-accept{background:#F2140A;color:#fff;border:none;border-radius:999px;padding:0.55rem 1.25rem;font-family:inherit;font-weight:600;font-size:0.9rem;cursor:pointer;transition:background .2s;}', '#cookie-note .cn-accept:hover{background:#C0140A;}', '#cookie-note .cn-decline{background:none;border:none;color:#A79F94;font-family:inherit;font-size:0.9rem;cursor:pointer;padding:0.55rem 0.5rem;}', '#cookie-note .cn-decline:hover{color:#F4F0E9;}', '@media (prefers-reduced-motion:reduce){#cookie-note{transition:none;transform:none;}}'].join('');
    var st = document.createElement('style');
    st.id = 'cookie-note-css';
    st.textContent = css;
    document.head.appendChild(st);
  }
  function bannerCopy() {
    if (LANG === 'lt') {
      return FB_PIXEL_ID ? 'Pora neprivalomų slapukų — analitika skaičiuoja apsilankymus, rinkodara matuoja reklamą. Tavo pasirinkimas. <a href="' + PRIVACY_HREF + '">Plačiau</a>' : 'Vienas neprivalomas slapukas — analitikai: skaičiuoja apsilankymus, nieko daugiau. <a href="' + PRIVACY_HREF + '">Plačiau</a>';
    }
    return FB_PIXEL_ID ? 'A couple of optional cookies \u2014 analytics to count visits, marketing to measure ads. Your call. <a href="' + PRIVACY_HREF + '">Details</a>' : 'One optional cookie, for analytics \u2014 it counts visits, nothing more. <a href="' + PRIVACY_HREF + '">Details</a>';
  }
  function hideBanner() {
    if (!banner) return;
  ...
  function showBanner() {
    ...
    banner.id = 'cookie-note';
    banner.setAttribute('role', 'dialog');
    banner.setAttribute('aria-label', LANG === 'lt' ? 'Pranešimas apie slapukus' : 'Cookie notice');
    banner.innerHTML = '<p>' + bannerCopy() + '</p>' + '<div class="cn-actions">' + '<button type="button" class="cn-accept">' + (LANG === 'lt' ? 'Sutinku' : 'Accept') + '</button>' + '<button type="button" class="cn-decline">' + (LANG === 'lt' ? 'Atsisakau' : 'Decline') + '</button>' + '</div>';
    document.body.appendChild(banner);
    banner.querySelector('.cn-accept').addEventListener('click', function () {
      window.vzConsent.accept();
    });
    banner.querySelector('.cn-decline').addEventListener('click', function () {
      window.vzConsent.reject();
    });
  ...
  function fireChange() {
    try {
      document.dispatchEvent(new CustomEvent('vzconsentchange', {
        detail: readState()
      }));
    } catch (e) {}
  }
  window.vzConsent = {
    ...
    set: function (cat, val) {
      ...
      s[cat] = val;
      writeState(s);
      if (val === 'granted' && LOADERS[cat]) LOADERS[cat]();
      if (val === 'denied') clearCookies(cat);
      fireChange();
    },
    status: function () {
      return readState();
  ...
  /* ---------- GA4 custom events (queued via gtag stub; sent only if analytics consented) ---------- */
  function bindEvents() {
    document.addEventListener('click', function (ev) {
      ...
      if (href.indexOf('booktomas.com') !== -1) {
        gtag('event', 'click_primary_cta', {
          link_text: label,
          page_path: location.pathname
        });
      } else if (a.classList.contains('textlink')) {
        gtag('event', 'click_secondary_cta', {
          link_text: label,
          page_path: location.pathname
        });
      }
  ...
  function init() {
    var s = readState();
    var undecided = CATS.some(function (c) {
      return s[c] !== 'granted' && s[c] !== 'denied';
    });
    applyState();
    if (undecided) showBanner();
    bindEvents();
  }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', init);else init();
})();
})(); } catch (e) { __ds_ns.__errors.push({ path: "consent.js", error: String((e && e.message) || e) }); }

// publish/assets/icons.js
try { (() => {
/* vision.lt icon set — lucide 0.460.0 subset, self-hosted */
(function () {
  var I = {
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "publish/assets/icons.js", error: String((e && e.message) || e) }); }

// publish/assets/image-slot.js
try { (() => {
// @ds-adherence-ignore -- omelette starter scaffold (raw elements/hex/px by design)
/* BEGIN USAGE */
/**
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "publish/assets/image-slot.js", error: String((e && e.message) || e) }); }
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "publish/consent.js", error: String((e && e.message) || e) }); }
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "vedlys/doc-page.js", error: String((e && e.message) || e) }); }

// vedlys/tweaks-panel.jsx
try { (() => {
// @ds-adherence-ignore -- omelette starter scaffold (raw elements/hex/px by design)
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "vedlys/tweaks-panel.jsx", error: String((e && e.message) || e) }); }
